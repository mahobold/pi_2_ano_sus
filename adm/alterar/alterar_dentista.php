...
if (isset($_POST['id_pessoadentista'])) {
    // Recupere os dados do POST
    $id_pessoadentista = $_POST['id_pessoadentista'];
    $data = $_POST['data'];
    $hora= $_POST['hora'];


    // Usando prepared statements para atualizar os dados
    $stmt = $mysqli->prepare("UPDATE dentista SET data = ?, hora = ? WHERE id_pessoadentista = ?");
    $stmt->bind_param('sss', $data, $hora, $id_pessoadentista);
...
if (isset($_GET["id_pessoadentista"])) {
    ...
    $stmt = $mysqli->prepare("SELECT * FROM dentista WHERE id_pessoadentista = ?");
    $stmt->bind_param('s', $id_pessoadentista);
    $stmt->execute();
...
<head>
    ...
    <title>Alterar - Consulta Dentista</title>
...
<body>
    ...
    <div class="container">
        <form action="" method="post">
            ...
            <input type="hidden" name="id_pessoadentista" value="<?php if (isset($consultar['id_pessoadentista'])){echo $consultar['id_pessoadentista'];}?>">
...
<script>
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "../consultas/consultas_dentista.php";  // Redirecione para a página desejada após o usuário clicar em "Ok"
        }
    })
</script>
<?php endif; ?>
