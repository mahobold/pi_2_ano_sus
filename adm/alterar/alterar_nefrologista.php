...
if (isset($_POST['id_nefro'])) {
    // Recupere os dados do POST
    $id_nefro = $_POST['id_nefro'];
    $data = $_POST['data'];
    $hora= $_POST['hora'];


    // Usando prepared statements para atualizar os dados
    $stmt = $mysqli->prepare("UPDATE nefro SET data = ?, hora = ? WHERE id_nefro = ?");
    $stmt->bind_param('sss', $data, $hora, $id_nefro);
...
if (isset($_GET["id_nefro"])) {
    ...
    $stmt = $mysqli->prepare("SELECT * FROM nefro WHERE id_nefro = ?");
    $stmt->bind_param('s', $id_nefro);
    $stmt->execute();
...
<head>
    ...
    <title>Alterar - Consulta Nefrologista</title>
...
<body>
    ...
    <div class="container">
        <form action="" method="post">
            ...
            <input type="hidden" name="id_nefro" value="<?php if (isset($consultar['id_nefro'])){echo $consultar['id_nefro'];}?>">
...
<script>
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "../consultas/consultas_nefrologista.php";  // Redirecione para a página desejada após o usuário clicar em "Ok"
        }
    })
</script>
<?php endif; ?>
