...
if (isset($_POST['id_pessoaoftal'])) {
    // Recupere os dados do POST
    $id_pessoaoftal = $_POST['id_pessoaoftal'];
    $data = $_POST['data'];
    $hora= $_POST['hora'];


    // Usando prepared statements para atualizar os dados
    $stmt = $mysqli->prepare("UPDATE oftal SET data = ?, hora = ? WHERE id_pessoaoftal = ?");
    $stmt->bind_param('sss', $data, $hora, $id_pessoaoftal);
...
if (isset($_GET["id_pessoaoftal"])) {
    ...
    $stmt = $mysqli->prepare("SELECT * FROM oftal WHERE id_pessoaoftal = ?");
    $stmt->bind_param('s', $id_pessoaoftal);
    $stmt->execute();
...
<head>
    ...
    <title>Alterar - Consulta Oftalmologista</title>
...
<body>
    ...
    <div class="container">
        <form action="" method="post">
            ...
            <input type="hidden" name="id_pessoaoftal" value="<?php if (isset($consultar['id_pessoaoftal'])){echo $consultar['id_pessoaoftal'];}?>">
...
<button type="button" class="btn btn-outline-primary" href="../consultas/consultas_oftalmologista.php">Voltar</button>
...
<script>
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "../consultas/consultas_oftalmologista.php";  // Redirecione para a página desejada após o usuário clicar em "Ok"
        }
    })
</script>
<?php endif; ?>
