...
if (isset($_POST['id_pessoacardio'])) {
    // Recupere os dados do POST
    $id_pessoacardio = $_POST['id_pessoacardio'];
    $data = $_POST['data'];
    $hora= $_POST['hora'];
...
if (isset($_GET["id_pessoacardio"])) {
    ...
    $stmt = $mysqli->prepare("SELECT * FROM cardio WHERE id_pessoacardio = ?");
    $stmt->bind_param('s', $id_pessoacardio);
    $stmt->execute();
...
<head>
    ...
    <title>Alterar - Consulta Cardiologista</title>
