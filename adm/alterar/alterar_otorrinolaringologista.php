...
if (isset($_POST['id_pessoaotorrino'])) {
    // Recupere os dados do POST
    $id_pessoaotorrino = $_POST['id_pessoaotorrino'];
    $data = $_POST['data'];
    $hora= $_POST['hora'];


    // Usando prepared statements para atualizar os dados
    $stmt = $mysqli->prepare("UPDATE otorrino SET data = ?, hora = ? WHERE id_pessoaotorrino = ?");
    $stmt->bind_param('sss', $data, $hora, $id_pessoaotorrino);
...
if (isset($_GET["id_pessoaotorrino"])) {
    ...
    $stmt = $mysqli->prepare("SELECT * FROM otorrino WHERE id_pessoaotorrino = ?");
    $stmt->bind_param('s', $id_pessoaotorrino);
    $stmt->execute();
...
<head>
    ...
    <title>Alterar - Consulta Otorrinolaringologista</title>
...
<body>
    ...
    <div class="container">
        <form action="" method="post">
            ...
            <input type="hidden" name="id_pessoaotorrino" value="<?php if (isset($consultar['id_pessoaotorrino'])){echo $consultar['id_pessoaotorrino'];}?>">
...
<button type="button" class="btn btn-outline-primary" href="../consultas/consultas_otorrinolaringologista.php">Voltar</button>
...
<script>
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = "../consultas/consultas_otorrinolaringologista.php";  // Redirecione para a página desejada após o usuário clicar em "Ok"
        }
    })
</script>
<?php endif; ?>
